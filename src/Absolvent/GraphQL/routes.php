...
], config('graphql.route_group_attributes', [])), function($router)
{
    ...
    // Controllers
    $controllers = config('graphql.controllers', \Absolvent\GraphQL\GraphQLController::class . '@query');
        ...
        if(preg_match($schemaParameterPattern, $queryRoute))
        {
            ...
            foreach(config('graphql.schemas') as $name => $schema)
            {
                foreach (array_get($schema, 'method', ['get', 'post']) as $method) {
                    $route = $router->{$method}(
                        Absolvent\GraphQL\GraphQL::routeNameTransformer($name, $schemaParameterPattern, $queryRoute),
                        [
                            'uses'          => $queryController,
    ...
    if($mutationRoute)
    {
        if(preg_match($schemaParameterPattern, $mutationRoute))
        {
            ...
            foreach(config('graphql.schemas') as $name => $schema)
            {
                foreach (array_get($schema, 'method', ['get', 'post']) as $method) {
                    $route = $router->{$method}(
                        Absolvent\GraphQL\GraphQL::routeNameTransformer($name, $schemaParameterPattern, $mutationRoute),
                        [
                            'uses'          => $mutationController,
                            'middleware'    => array_get($schema, 'middleware', []),
                        ]
...
if (config('graphql.graphiql.display', true))
{
    ...
    {
        $graphiqlController =  config('graphql.graphiql.controller', \Absolvent\GraphQL\GraphQLController::class . '@graphiql');
        $schemaParameterPattern = '/\{\s*graphql\_schema\s*\?\s*\}/';
        $graphiqlAction = ['uses' => $graphiqlController];

        foreach (config('graphql.schemas') as $name => $schema)
        {
            $route = $router->get(Absolvent\GraphQL\GraphQL::routeNameTransformer($name, $schemaParameterPattern, '{graphql_schema?}'), $graphiqlAction);
            if (! is_lumen()) {
                $route->where($name, $name);
            }

            $route = $router->post(Absolvent\GraphQL\GraphQL::routeNameTransformer($name, $schemaParameterPattern, '{graphql_schema?}'), $graphiqlAction);
            if (! is_lumen()) {
                $route->where($name, $name);
            }
        }
